...
const ClientEquipments = () => {
  ...
  const handleEdit = (locationId) => {
    navigate(`/edit-client-equipment/${locationId}`);
  };
  ...
  return (
    <>
      ...
      <div className="flex">
        ...
        <h2 className="text-xl font-semibold mb-2">Client Equipments</h2>
        <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col mb-4">
            <label htmlFor="client" className="mr-2 text-xl font-semibold">Select Client:</label>
            <select
              id="client"
          ...
          <div className="flex flex-col mb-4">
            <label htmlFor="client" className="mr-2 text-xl font-semibold">Select Location:</label>
            <select
              id="client"
              className="border border-gray-300 rounded px-3 py-1 w-full"
              onChange={(e) => handleClientChange(e.target.value)}
        ...
        <div className="flex justify-between items-center">
              <div className="flex gap-4 w-[70%]">
                <div className="flex flex-col gap-2">
                  <label className="font-normal text-sm">Filter By Device Status</label>
                  <select
                    name="status"
                  ...
                  >
                    ...
                    <option value="Open">Retired</option>
                  </select>
                </div>
                <div className="flex flex-col gap-2">
          ...
          {selectedClient !== null && (
            <div className="mb-4">
              <h2 className="text-xl font-semibold mb-2">Devices for {clients?.data?.find(client => client.client_id === selectedClient)?.company_name}</h2>
              <div className="flex justify-end mb-2">
                <button className="bg-indigo-700 text-white px-4 py-2 rounded"><Link to={`/add-client-equipment/${selectedClient}`}>Add New Equipment</Link></button>
              </div>
              {loadinglocations ? (
                <p>Loading client devices...</p>
              ) : (
