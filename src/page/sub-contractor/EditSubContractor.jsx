...
import { 
  getSubcontractorTyes, 
  getSubcontractorServices, 
...
const EditSubContractor = () => {
  ...
  const { id } = useParams();
  const { loading,loadingDetails } = useSelector((state) => state.subcontractor);
//   const { user_type } = useSelector((state) => state.user.user);
  const { subcontractorTypes, subcontractorServices, subcontractor } = useSelector((state) => state.subcontractor);
  ...
  useEffect(() => {
    ...
    dispatch(getSubcontractorDetails(id));
  }, [dispatch, id]);

  // Populate form when subcontractor data loads
  useEffect(() => {
    if (subcontractor) {
      setFormData({
        subcontractorName: subcontractor.subcontractor_name,
        streetAddress: subcontractor.street_address,
        city: subcontractor.city,
        state: subcontractor.state,
        zipCode: subcontractor.zipcode,
        coverageArea: subcontractor.coverage_area,
        hourlyRate: subcontractor.hourly_rate,
        tripCharge: subcontractor.trip_charge,
        numberOfTechnicians: subcontractor.no_of_technicians,
        ownsCertifier: subcontractor.is_certifier ? "Yes" : "No",
        contractorTypes: subcontractor.contractor_provided_types.map(t => ({
          type_id: t.type_id,
          type_name: t.type_name
        })),
        servicesProvided: subcontractor.contractor_provided_services.map(s => ({
          service_id: s.service_id,
          service_name: s.service_name
        })),
        additionalNotes: subcontractor.additional_notes,
        projectContact: {
          firstName: subcontractor.p_firstname,
          lastName: subcontractor.p_lastname,
          phoneNumber: subcontractor.p_phonenumber,
          mobileNumber: subcontractor.p_mobilenumber,
          email: subcontractor.p_email
        },
        serviceContact: {
          firstName: subcontractor.s_firstname,
          lastName: subcontractor.s_lastname,
          phoneNumber: subcontractor.s_phonenumber,
          mobileNumber: subcontractor.s_mobilenumber,
          email: subcontractor.s_email
        },
        accountsReceivableContact: {
          firstName: subcontractor.a_firstname,
          lastName: subcontractor.a_lastname,
          phoneNumber: subcontractor.a_phonenumber,
          mobileNumber: subcontractor.a_mobilenumber,
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateSubcontractor(payload, navigate));
  };
    ...
    <>
      ...
      <div className="flex">
        ...
        <div className="container mx-auto p-4 w-full h-screen overflow-y-scroll">
          <h1 className="text-2xl font-bold mb-4">Edit SubContractor</h1>
          
          {loadingDetails ? (
            <div>Loading subcontractor data...</div>
          ) : (
            <>
        ...
        <div className="flex flex-col mb-4">
          ...
          >
            ...
            <option value="F">F</option>
              ...
              >
               {loading ? "Saving Data"  : "Update SubContractor"}
              </button>
            </form>
            <SubcontractorNotes 
            subcontractorId={id}
